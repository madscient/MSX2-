...
#ifdef _DOPPEL_VDP
	vdp2 = new V99X8(this, emu);
	vdp_mixer = new vdp_mixer_device(this, emu, vdp, vdp2);
#endif
#else
...
#ifdef _DOPPEL_VDP
	vdp_mixer->set_context_irq(cpu, SIG_CPU_IRQ, 1);
#else
...
#ifdef _DOPPEL_VDP
	memory->set_context_vdp(vdp_mixer);
#else
...
#ifdef _DOPPEL_VDP
	io->set_iomap_range_rw(0x98, 0x9b, vdp_mixer);
	io->set_iomap_range_rw(0x88, 0x8b, vdp_mixer);
	io->set_iomap_single_rw(0xb8, vdp_mixer);
#else
